...
# এরপর আপনার বর্তমান কোডগুলো থাকবে (অর্থাৎ ২ নম্বর লাইন থেকে যা ছিল)
import asyncio
import io
import re
import json
...
BOT_TOKEN = "8973417435:AAGyUbsqi-8Sc9OSKJC4DQFqoOBKmlqZFVA"
API_KEY = "mino_live_43b5cf7151b4c17c0a14623cb5085ede"  
BASE_URL = "https://mino-sms-panel.xyz"      

# --- SYSTEM IDS & ADMINS ---
ADMINS = [7323145435]
OTP_GROUP_ID = -1004360700957

# --- SYSTEM LINKS & USERNAME SETTINGS ---
DEFAULT_WELCOME_MESSAGE = f"{p_em('live')} <b>GANJA OTP BOT</b> {p_em('live')}\n━━━━━━━━━━━━━━━━━━━━━━━━\n{p_em('status')} <b>START INSTANT OTP RECEPTION NOW!</b> {p_em('status')}\n━━━━━━━━━━━━━━━━━━━━━━━━"
DEFAULT_OTP_GROUP_URL = "https://t.me/ganja_otp_group"
DEFAULT_CHANNEL_URL = "https://t.me/GANJAOTP"
DEFAULT_SUPPORT_USERNAME = "@GANJABOSS_1"
FORCE_JOIN_CHANNELS = ["@GANJAOTP"]

# --- WITHDRAWAL & STATS LIMITS ---
DEFAULT_MIN_WITHDRAW = 2.5
DEFAULT_MAX_WITHDRAW = 100.0
DEFAULT_COOLDOWN_TIME = 1.0
DEFAULT_OTP_REWARD = 0.0010
DEFAULT_REFER_BONUS = 0.040
DEFAULT_NUMBERS_PER_REQUEST = 2
...
async def show_traffic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not os.path.exists(ACTIVITY_LOGS_FILE):
        await update.message.reply_text(
            f"{p_em('live')} <b>Live Traffic (Last 3 Hours)</b>\n\n<i>No traffic logs recorded yet.</i>",
            parse_mode="HTML"
        )
        return
...
async def support_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [rbtn("SUPPORT 2", "primary", url="https://t.me/Parvez_payel", icon_custom_emoji_id=EMOJI_ID_MAP.get("link"))]
    ])
                ...
                for otp in otps:
                    ...
                    if (matched_key is not None and
                        ...
                        user_msg = (
                            ...
                            f"<b>━━━━━━━━━━━━━━━━━━━━━━━━━━</b>\n"
                            f"<b>☎️ Number :</b> <code>{full_number}</code>\n"
                            f"<b>🔑 OTP    :</b> <code>{safe_otp_code}</code> ✍️\n"
                            f"<b>🤑 Bonus  :</b> <code>+{otp_reward:.4f}$ Credited</code>\n"
                        )

                        group_msg = (
                            f"<b>━━━━━━━━━━━━━━━━━━━━━━━━</b>\n"
                            f"🔴<b>GANJA BOT LIVE OTP RECEIVED</b>🔴\n"
                            f"<b>━━━━━━━━━━━━━━━━━━━━━━━━</b>\n"
                            f"<b>📱 App Service :</b> <code>{service_name.upper()}</code>\n"
                            f"☎️<b> Mobile No :</b> <code>{masked_number}</code>\n"
                            f"🌍<b> Country  :</b> {country_flag} {country_name}\n"
                            f"<b>🔑 OTP Code :</b> <code>{safe_otp_code}</code>\n"
                        )
                        ...
                        group_buttons = InlineKeyboardMarkup([
                            [
                                rbtn("📢 CHANNEL", "primary", url=settings.get("channel_url", DEFAULT_CHANNEL_URL), icon_custom_emoji_id="5350396951407895212"),
                                rbtn("🤖NUMBER BOT🤖", "success", url="https://t.me/""ganja_otp_bot", icon_custom_emoji_id="5271604874419647061")
                            ]
                        ])
...
async def process_auto_number(update, context, range_text):
    ...
    try:
        ...
        final_text = (
            f" ☎️ <b>YOUR ACTIVE NUMBER DETAILS</b> ☎️\n"
            f"<b>━━━━━━━━━━━━━━━━━━━━━━━━</b>\n"
            f"<b>📱 App :</b> <code>{context.user_data.get('selected_app', 'Custom Range')}</code>\n"
            f"<b>🌍 Country :</b> {country_flag} <code>{country_name}</code>\n"
            f"<b>☎️ Number  :</b> <code>+{generated_num}</code>\n"
            f"<b>━━━━━━━━━━━━━━━━━━━━━━━━</b>\n"
            f"⏱️<b>SMS STATUS:</b> Waiting for message... 🥱\n"
        )
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "same_range":
        ...
        txt = (
            ...
            f"<b>━━━━━━━━━━━━━━━━━━━━━━━━</b>\n"
            f"<b>📱 App :</b> <code>{app_name}</code>\n"
            f"<b>🌍 Country :</b> {country_flag} <code>{country_name_local}</code>\n"
            + "\n".join(num_lines) + "\n"
            f"<b>━━━━━━━━━━━━━━━━━━━━━━━━</b>\n"
            f"⏱️<b>SMS STATUS:</b> Waiting for message... 🥱\n"
        )
